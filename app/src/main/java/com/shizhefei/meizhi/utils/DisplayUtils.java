	...
	public static int getDeviceType(Context context) {
		if (deviceType == -1) {
			if (context.getPackageManager().hasSystemFeature("android.hardware.telephony")) {
				LogUtils.d("Mobile Test", "Running on phone");
				// Check if android.hardware.touchscreen feature is available.
				deviceType = 1;

			} else if (context.getPackageManager().hasSystemFeature("android.hardware.touchscreen")) {
				LogUtils.d("Tablet Test", "Running on devices that don't support telephony but " + "do have a touch screen.");
				deviceType = 2;

			} else {
				LogUtils.d("TV Test", "Running on a TV!");
				deviceType = 3;
			}
		}
